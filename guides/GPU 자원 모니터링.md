...
Token 값은 사용자를 식별하기 위한 해시 값이며 사용자마다 고유 값을 가지고 있습니다. Token은 일반적으로 **사용자의 가상 머신(VM) 안에 위치하며, HAC 서버는 Token 값을 바탕으로 사용자를 식별하고 학습이 실행되므로,** Token 없이는 GPU 연산 및 Python 애플리케이션을 실행할 수 없습니다.
